...
class Window(pyglet.window.Window):
    def __init__(self, *args, **kwargs):
        ...
        self.generation_label = pyglet.text.Label('test',
                                                  font_name='Arial', color=(0, 0, 0, 255),
                                                  font_size=12,
                                                  x=WIDTH*0.05, y=HEIGHT*0.95, batch=self.main_batch)
        self.best_fitness_label = pyglet.text.Label('test',
                                                    font_name='Arial', color=(0, 0, 0, 255),
                                                    font_size=12,
                                                    x=WIDTH*0.05, y=HEIGHT*0.9, batch=self.main_batch)
        self.obstacles = []
        self.set_obstacles()
    ...
    def set_obstacles(self):
        ...
        obstacle = Obstacle(position=(start_point+end_point),
                            width=3, batch=self.main_batch)
    ...
    def update(self, dt):
        ...
        if(self.population.best_fitness_not_increased_count > 0):
            self.best_fitness_label.text = 'Best Fitness: ' + \
                str(self.population.best_fitness)
        else:
            self.best_fitness_label.text = 'Best Fitness: ' + \
                str(self.population.best_fitness)+' (evolving)'
